...
class StartScreenViewController: UIViewController, UITextFieldDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        PersistenceService.shared.fetchDrinks()
                ...
                if((self.loginTextField?.text)! != "" && (self.passwordTextField?.text)! != "")
                {
                    if(PersistenceService.shared.checkAccount(username: (self.loginTextField?.text)!))
                    {
                        if(PersistenceService.shared.checkAccountPassword(password: (self.passwordTextField?.text)!))
                        {
                            ...
                            print("LOGIN")
                            print(PersistenceService.shared.currentLoggedInUserInfo?.username)
                            print(PersistenceService.shared.currentLoggedInUserInfo?.gender)
                            print(PersistenceService.shared.currentLoggedInUserInfo?.weight)
                            print("LOGIN")
                        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showSettings"
        {
            ...
            //let accountVC = vc.childViewControllers[0] as! AccountSettingsViewController
